...
9. **真实训练入口下浮点不逐位可复现，所以关闭态等价性必须用精确量判定，不能用「差异很小」。**
   ...
   **订正（2026-08-05，第四轮：终于实测了）**：同一配置带探针 / 不带探针各跑一次，
   差值是 **+0.98 MiB（+0.010%）**，比那个「6 MiB 量级」小一个数量级，
   而且**落在 ~5 MiB 的 run-to-run 噪声里**。所以正确说法是
   「观测器开销在这套测量下**不可分辨**，上界约 5 MiB」。
   **那个 6 MiB 是推断，被写成了实测** —— 与峰值显存被当成精确量是同一个毛病：
   **一个数在被引用之前，先得知道它的噪声地板在哪。**
   → 这一条本身也升级成动作：**「观测器污染」不要靠估，跑一对 run 把它测出来**
