...
@register_adapter("table30v2")
class Table30v2Adapter(DatasetAdapter):
    """Adapter for entries with ``entry["kinematics"]["urdf"]``."""

    packer_module = "config_table30v2_dataset"

    def resolve_urdf_path(self, entry: Mapping[str, Any]) -> Optional[str]:
        kinematics = entry.get("kinematics")
