...
def update_submit_json(
    ...
    config["job_name"] = (
        f"arrow_dataset_pack_{dataset_name}_startid_{start_idx}_endid_{end_idx}_InternData_A1_data"  # noqa: E501
    )
    config["to_upload"] = [
        "robo_orchard_lab",
        "projects/sem/common/configs/config_interna1_dataset.py",
        "projects/sem/common/configs/config_sem_common.py",
        "robo_orchard_lab/dataset/interna1/packer/arrow_packer/arrow_pack_InternA1.py",
        "robo_orchard_lab/dataset/interna1/packer/arrow_packer/viz_arrow_InternA1.py",
        script_file,
    ]
...
def update_script_shell(
    ...
    Args:
        script_file (str, optional): Path to the shell script file.
            Defaults to "run_pack.sh".
        arrow_output_path (str, optional): Output path for the Arrow dataset.
            Defaults to None.
    ...
    script_content = f"""#!/bin/bash
        ...
        python3 arrow_pack_InternA1.py \
        --input_path {task_dir} \
        --output_path {arrow_output_path} \
        --start_idx {start_idx} \
        --end_idx {end_idx} \

        python3 viz_arrow_InternA1.py \
        --arrow_dataset_path {arrow_output_path} \
        --output_path {os.path.join(arrow_output_path, "viz")}
...
if __name__ == "__main__":
    ...
    arrow_output_root = "/horizon-bucket/robot_lab2/users/yun01.du/data/InternData_A1_arrow_0125"  # noqa: E501
    ...
    for task_idx, task_info in enumerate(total_task_dir):
        ...
        for i in range(0, total_episodes, each_size):
            ...
            arrow_output_path = os.path.join(
                arrow_output_root,
                f"{robot_type}/arrow_dataset_{dataset_name}_ep_startid_{start_idx}_ep_endid_{end_idx}",
            )
            print(arrow_output_path)
