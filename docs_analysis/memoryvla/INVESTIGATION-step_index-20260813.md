> ⚠️ **本文件的主结论已于 2026-08-18 被推翻。**
> 其中所有评测数字都是在 **RGB 通道错配**下测得的。通道修正后:
> stride32 × 修复编号 = **93/100**,而此前最好的格子是 22/100(p=2.5e-26)。
> 「stride32 更差」「记忆库旋钮全零」「时间配置不是瓶颈」三条**均不成立**。
> 见 `RESULT-93pct-20260818.md`。机制分析(PE 只加在 key 上、aliasing 算式)仍然成立。
