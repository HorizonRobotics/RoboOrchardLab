...
9. **真实训练入口下浮点不逐位可复现，所以关闭态等价性必须用精确量判定，不能用「差异很小」。**
   ...
   本次的探针每个 forward clone 两个张量，**把 D 档显存读数抬高了 6 MiB 量级**。
   ~~**订正（2026-08-05，第四轮：终于实测了）**：差值是 **+0.98 MiB（+0.010%）**，
   落在 ~5 MiB 的 run-to-run 噪声里，所以「观测器开销**不可分辨**，上界约 5 MiB」。~~

   **订正的订正（2026-08-05，第五轮，复审 P2-E）：第四轮那个订正本身是错的，已撤回。**
   第四轮那对 run **跨卡**（带探针在 gpu 3、不带在 gpu 0），而且用来判「落在噪声里」的
   ~5 MiB 噪声地板是从**关闭态** run 测出来的，属于**另一个量**。复审第四轮用四个 run 重测：

   ```
   带探针   gpu1  9.305203437805176      gpu4  9.302354335784912
   不带     gpu1  9.297196865081787      gpu5  9.297196865081787   ← 跨卡逐位相同
   同卡（gpu1）对照差值 = +8.01 MiB
   ```

   **两个不带探针的 run 跨卡逐位相同 ⇒ 不带探针的值稳定、不是噪声**；
   带探针两次相差 2.92 MiB，**小于要测的差值**。
   ⇒ **观测器开销可分辨，量级 5–8 MiB。原来那句「6 MiB 量级」落在实测区间内。**

   **这一条现在有三层教训，第三层才是最贵的：**
   1. 一个数在被引用之前，先得知道它的噪声地板在哪；
   2. **噪声地板必须来自同一个量、同一套条件** —— 拿 A 的地板去判 B 的差值，
      会把一个真实效应判成噪声（第四轮就是这么把对的结论划掉的）；
   3. **对照必须控制住卡**。同一配置在不同卡上的峰值显存不可直接相减。
   → 动作：**「观测器污染」不要靠估，跑一对 run 把它测出来，而且两个 run 必须在同一张卡上**
   （runner 留一个 `--no-identity` 之类的开关，成本是一次 run）。
   → 一条更一般的：**「这个差值落在噪声里」是一个需要证据的断言，不是一个可以顺手下的判断** ——
   证据是「同条件重复测量的散布 ≥ 待判差值」，而不是别处一个看起来差不多大的数。
...
11. **护栏挂在生产端会被整体绕过 —— 要挂在消费端，判据用可观测的后果，不用配置项的名字。**
    ...
    - **报错文案本身会制造事故。** 那条 raise 写着「the episode sampler is only meaningful
      ...
      **订正（2026-08-05，第四轮）**：写下这句时，那些断言在
      `$ROL_JFS/port/memoryvla/fix3/guard3_unit_test.py` 里，**不在 git 里**，
      仓内没有任何 runner 会执行它们 —— 「可以回归测试」当时只是**能力陈述**，
      被写成了**已完成陈述**。教训本身成立，落地要看断言在不在会被执行的入口上。
...
12. **能在构造期静态判定的前提，不要留给运行期的后果判据兜底。后果判据是兜底，不是唯一。**
    教训 11 把判据从「配置项的名字」换成「可观测的后果」，方向对，但**换过头了**：
    有一类前提**根本不需要运行就能判**，把它们也推给后果判据，等于白白引入一段无保护的窗口。
    本次的形态：`dataloader_type="group"` 每次训练调用清空 bank、批内每 `group_size`
    个样本再清一次，所以记忆跨度恒等于 `min(group_size, batch_size)`；等于 1 时
    **没有任何样本有前驱可读**。这是两个配置值加一个 batch size 的算术，
    而它被留给了「跑满 K 次 forward 看 bank 涨没涨」。
    → **判据分两层**：能从配置 + 装配结果算出来的，在装配期 fail-fast；
    只有真跑起来才知道的（比如「批次实际是不是 episode 连续」），才交给后果判据。
    → 一个可操作的问法：**「这个失效，需要看到数据才能确定吗？」**
    不需要，就不该等到 forward。

13. **带计数 / 时间闸门的判据，必须交付「闸门未到达时的行为」这一档证据。**
    「跑满 K 次 forward 后如果 X 从没发生过就 raise」听起来是全覆盖，
    实际上它对**所有短于 K 的运行**完全无效 —— 而冒烟恰恰是短跑：
    本项目自己的消融矩阵整张表 8 step，上一轮的对照 run 4 step，K 取的是 8。
    于是「护栏有牙」的证据（12 步会 raise）与「日常真的会被保护」是两回事。
    → **验收表里要有独立的一行：闸门未到达那一档，逐个配置回答「谁在看守」。**
    答不上来的格子就是洞，要么补一道不带闸门的判据，要么如实写进遗留。
    → 同一条也适用于「第一次才检查」「每 N 步检查一次」「采样检查」这类判据。

14. **一条判据分辨不了的成因，它的报错文案就不能断言；更不能建议一个在当前场景下已经生效的动作。**
    `bank 恒为 1` 有两种成因：批次坏了，或这个配置下记忆本来就不可能。
    判据只看得见后果，**看不见成因**，而文案断言了前者，并建议
    `episode_stream_sampler=True` —— 在唯一真正触发它的那个配置里，那个开关**已经是 True**。
    使用者照做会发现无事可做，只能去读源码。这是教训 11「报错文案本身会制造事故」的加重版：
    上一次是**指错方向**，这一次是**给了一个空操作**。
    → **文案的结构固定成：先报观测值，再列成因与各自的动作。**
    观测值让读者能自己判断落在哪一支；「列出两条」比「断言一条」诚实，也更短。
    → 一条机械判据：**任何建议，都要能回答「在触发它的那个场景里，照做会发生什么变化」。**
    答案是「什么都不变」，那句建议就是错的。

15. **证据文件从第一次写就要绑定代码哈希；中途才加 = 早期证据全部不可采信，且很难事后补。**
    上一轮的 provenance 是会话中途才加进 runner 的，结果**唯一一份 stream 开启态回归证据**
    没有任何代码绑定，只能靠 mtime 猜。更隐蔽的是：所有 run 记录的 `git_head`
    都是**前一个** commit（代码还没提交、`git_dirty=True`），
    要不是 runner 顺带记了逐文件 sha256，整批证据都绑不到被审的那个 commit 上。
    → **runner 第一版就写 provenance**：`git_head` + `git_dirty` + 被跑文件的 sha256
    （**按磁盘读，不要按 `sys.modules` 读** —— 关闭态根本不 import，那样会得到空表）。
    → **顺序上：先提交，再跑验证。** 这样每份证据都是 `git_dirty=False` 且
    `git_head == 被审 commit`，绑定是同一性而不是推断。
    代价是「先写文档再提交」做不到，那就把文档回填拆成单独一个提交——
    比让整批证据长在脏树上便宜得多。
    → 交付前跑一遍机械核对：**每份证据记录的文件哈希 == 被审 commit 对应文件的哈希**，
    对不上的要么重跑，要么标为不可采信。
