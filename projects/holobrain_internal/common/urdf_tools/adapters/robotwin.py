...
"""RoboTwin packer adapter.

The adapter exposes a single ``resolve_urdf_path`` hook that reads the URDF
path from a nested RoboTwin-schema config entry
(``entry["kinematics_config"]["urdf"]``). All other alignment intent (arm
chains, EE frame rotations, camera references) lives in the per-embodiment
``alignment.yaml`` manifests, so this adapter carries no discovery logic.
"""
...
class RobotwinAdapter(DatasetAdapter):
    """Adapter for the ``config_robotwin_dataset`` packer."""
