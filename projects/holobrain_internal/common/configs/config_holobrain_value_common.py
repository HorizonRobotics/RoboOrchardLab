...
config = dict(
    ...
    value_norm_mode="episode",
    output_dim=51,  # use L2Loss when set to 1
    loss_mode="hlgauss",
)
...
def build_model(config):
    ...
    from robo_orchard_lab.models.value_model import (
        HoloBrain_Value_Qwen2_5_VL,
        HoloBrain_Value_Qwen2_5_VLConfig,
        HoloBrainValueDecoder,
        HoloBrainValueLoss,
        ValueUpsampleHead,
    )
    ...
    if "qwen3" in config["vlm_pretrain"].lower():
        ...
        patch_size = 28
        model_class = HoloBrain_Value_Qwen2_5_VL
        model_config = HoloBrain_Value_Qwen2_5_VLConfig
    ...
    if config["output_dim"] == 1 and config["loss_mode"] != "l2":
        raise ValueError(
            f"When output_dim == 1, loss_mode must be 'l2',"
            f"got {config['loss_mode']}"
        )
    loss = dict(
        type=HoloBrainValueLoss,
        min_value=-1,
        max_value=0,
        num_bins=config["output_dim"],
